...
// ========================================
// COMMAND ACK TIMEOUT HANDLER
// ========================================
//
// Registered once at module load. When machineService's timeout
// fires (no ack within COMMAND_ACK_TIMEOUT_MS), this logs it to
// Postgres and tells any connected website viewers, using the
// exact same "command_ack" shape a real ack would use.
//
// ========================================
...
onCommandTimeout(
  (machineId, command) => {
    ...
    console.log(
      `⏱️  Command timeout [${machineId}]: no ack for "${command}"`
    );
    ...
    logCommandAck(
      ...
      console.error(
        `⚠️ Command timeout log failed [${machineId}]:`,
        error.message
      );
...
function setupMachineSocket(
  ...
  const wss =
    new WebSocket.Server({
      ...
      // No legitimate message (machine_state, command,
      // pairing info, logs) is anywhere near this size.
  ...
  wss.on(
    ...
    (ws) => {
      ...
      // ==================================
      //
      // WebSocket ping/pong is protocol-level — the RPi agent's
      // websocket-client library already answers PING frames
      // automatically (no RPi code changes needed). This just
      // tracks whether each connection is still answering.
      //
      // ==================================
  ...
  // ==================================
  //
  // Every 30s: ping every connection. If a connection didn't
  // answer the PREVIOUS ping with a pong (isAlive still false),
  // it's silently dead — terminate it. terminate() fires the
  // existing "close" handler, so all the normal disconnect
  // cleanup (Firestore offline state, website broadcast) runs
  // exactly as it does for a clean disconnect.
  //
  // ==================================
