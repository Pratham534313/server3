...
// ========================================
//
// If a command never gets acknowledged (RPi crashed mid-command,
// silent network drop, etc.), it should not sit as "sent"
// forever. After COMMAND_ACK_TIMEOUT_MS with no ack, it's marked
// "timeout".
//
// machineSocket.js registers a callback via onCommandTimeout() so
// it can log to Postgres / broadcast to the website when this
// fires — kept as a callback (not a direct require) to avoid a
// require cycle between machineService and machineSocket.
//
// ========================================
...
const pendingCommandTimers =
  new Map(); // machineId -> { timer, token }

const commandTimeoutHandlers =
  [];

function onCommandTimeout(
  callback
) {
  commandTimeoutHandlers.push(
    callback
  );
}
...
async function markCommandSent(
  ...
  // A fresh token for THIS specific send. The timeout callback
  // only acts if this token is still the current one when it
  // fires — so a newer command (or a real ack) always wins over
  // a stale timer.
  const token =
    Date.now() +
    "-" +
  ...
  // Replace any previous pending timer for this machine.
      ...
      async () => {
        ...
        if (
          ...
          try {
            ...
            console.error(
              `⚠️ Failed to persist command timeout [${machineId}]:`,
              error.message
            );

          }

          for (const handler of commandTimeoutHandlers) {
            ...
            try {
              ...
              console.error(
                "⚠️ Command timeout handler error:",
                error.message
              );
...
async function markCommandAck(
  ...
  // A real ack arrived — the pending timeout is no longer needed.
  clearPendingCommandTimer(
    machineId
  );
